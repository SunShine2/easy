...
;(function(){
    ...
    $.EasyTouch.ModelList = $.Base.build('$.EasyTouch.ModelList', {
            ...
            model: undefined,
            /**
             * 用于异步请求的参数，和`$.ajax`(http://zeptojs.com/#$.ajax)的参数一致，在扩展一个`Model`时可以事先定义一份基础的请求参数
             *
             *      {
             *          url: xxx,
             *          type: 'POST',
            ...
            next: function(options, settings){
                var _this = this;
                settings = settings || {};
                options = $.extend(_this.server, options);
            ...
            fetch: function(options, settings){
                settings = settings || {};
                options = $.extend(this.server, options);
                var _this = this,
