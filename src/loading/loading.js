...
(function($) {
  ...
  Loading.show = function (timeout, callback, str) {
    ...
    if (timeout && callback) {
      ...
      Loading.interval = setInterval(function() {
        ...
        if (time > timeout && loading.style.display !== 'none') {
          clearInterval(Loading.interval);
          Loading.interval = null;
          Loading.hide();
          callback();
  ...
  Loading.hide = function() {
    setTimeout(function(){
      var loading = document.getElementById('hb_loading');
      if (loading) {
